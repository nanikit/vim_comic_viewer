import {
  ...
  WidthProperty,
  ZIndexProperty,
} from "https://cdn.jsdelivr.net/gh/modulz/stitches/packages/core/src/css-types.ts";
...
export type TRecursiveCss<T extends TConfig, D = TFlatCSS<T>> =
  & (
    ...
    | {
      [pseudo: string]:
        & (D | { [pseudo: string]: (D | { [pseudo: string]: D }) & D })
        & D;
    }
  )
  & D;
...
export type TFlatCSS<T extends TConfig> = {
  [K in keyof Properties]?: K extends keyof ICssPropToToken<T>
    ? T extends { strict: true } ? ICssPropToToken<T>[K]
...
export type TFlatUtils<
  ...
  UT = {
    [U in keyof T["utils"]]?: T["utils"][U] extends TUtility<any, any>
      ? T["utils"][U] extends (arg: infer A, config: T) => {} ? A
      : never
      : never;
  },
> = UT;
...
export type ICssPropToToken<T extends TConfig> = T["tokens"] extends object ? {
  gap: T["tokens"]["space"] extends object ? keyof T["tokens"]["space"]
    : GridGapProperty<never>;
  gridGap: T["tokens"]["space"] extends object ? keyof T["tokens"]["space"]
    : GridGapProperty<never>;
  columnGap: T["tokens"]["space"] extends object ? keyof T["tokens"]["space"]
    : GridColumnGapProperty<never>;
  gridColumnGap: T["tokens"]["space"] extends object
    ? keyof T["tokens"]["space"]
    : GridColumnGapProperty<never>;
  rowGap: T["tokens"]["space"] extends object ? keyof T["tokens"]["space"]
    : GridRowGapProperty<never>;
  gridRowGap: T["tokens"]["space"] extends object ? keyof T["tokens"]["space"]
    : GridRowGapProperty<never>;
  inset: T["tokens"]["colors"] extends object ? keyof T["tokens"]["colors"]
    : InsetProperty<never>;
  insetBlock: T["tokens"]["colors"] extends object ? keyof T["tokens"]["colors"]
    : InsetBlockProperty<never>;
  insetBlockEnd: T["tokens"]["colors"] extends object
    ? keyof T["tokens"]["colors"]
    : InsetBlockEndProperty<never>;
  ...
  marginTop: T["tokens"]["space"] extends object ? keyof T["tokens"]["space"]
    : MarginTopProperty<never>;
  marginRight: T["tokens"]["space"] extends object ? keyof T["tokens"]["space"]
    : MarginRightProperty<never>;
  marginBottom: T["tokens"]["space"] extends object ? keyof T["tokens"]["space"]
    : MarginBottomProperty<never>;
  marginLeft: T["tokens"]["space"] extends object ? keyof T["tokens"]["space"]
    : MarginLeftProperty<never>;
  marginInline: T["tokens"]["space"] extends object ? keyof T["tokens"]["space"]
    : MarginInlineProperty<never>;
  ...
  marginBlock: T["tokens"]["space"] extends object ? keyof T["tokens"]["space"]
    : MarginBlockProperty<never>;
  marginBlockEnd: T["tokens"]["space"] extends object
    ? keyof T["tokens"]["space"]
    : MarginBlockEndProperty<never>;
  ...
  paddingTop: T["tokens"]["space"] extends object ? keyof T["tokens"]["space"]
    : PaddingTopProperty<never>;
  paddingRight: T["tokens"]["space"] extends object ? keyof T["tokens"]["space"]
    : PaddingRightProperty<never>;
  paddingBottom: T["tokens"]["space"] extends object
    ? keyof T["tokens"]["space"]
    : PaddingBottomProperty<never>;
  paddingLeft: T["tokens"]["space"] extends object ? keyof T["tokens"]["space"]
    : PaddingLeftProperty<never>;
  paddingInline: T["tokens"]["space"] extends object
    ? keyof T["tokens"]["space"]
    : PaddingInlineProperty<never>;
  ...
  paddingBlock: T["tokens"]["space"] extends object ? keyof T["tokens"]["space"]
    : PaddingBlockProperty<never>;
  ...
  top: T["tokens"]["space"] extends object ? keyof T["tokens"]["space"]
    : TopProperty<never>;
  right: T["tokens"]["space"] extends object ? keyof T["tokens"]["space"]
    : RightProperty<never>;
  bottom: T["tokens"]["space"] extends object ? keyof T["tokens"]["space"]
    : BottomProperty<never>;
  left: T["tokens"]["space"] extends object ? keyof T["tokens"]["space"]
    : LeftProperty<never>;
  ...
  borderLeftColor: T["tokens"]["colors"] extends object
    ? keyof T["tokens"]["colors"]
    : BorderLeftColorProperty;
  caretColor: T["tokens"]["colors"] extends object ? keyof T["tokens"]["colors"]
    : CaretColorProperty;
  color: T["tokens"]["colors"] extends object ? keyof T["tokens"]["colors"]
    : Color;
  ...
  outlineColor: T["tokens"]["colors"] extends object
    ? keyof T["tokens"]["colors"]
    : OutlineColorProperty;
  fill: T["tokens"]["colors"] extends object ? keyof T["tokens"]["colors"]
    : FillProperty;
  stroke: T["tokens"]["colors"] extends object ? keyof T["tokens"]["colors"]
    : StrokeProperty;
  fontFamily: T["tokens"]["fonts"] extends object ? keyof T["tokens"]["fonts"]
    : FontFamilyProperty;
  ...
  blockSize: T["tokens"]["sizes"] extends object ? keyof T["tokens"]["sizes"]
    : BlockSizeProperty<never>;
  minBlockSize: T["tokens"]["sizes"] extends object ? keyof T["tokens"]["sizes"]
    : MinBlockSizeProperty<never>;
  maxBlockSize: T["tokens"]["sizes"] extends object ? keyof T["tokens"]["sizes"]
    : MaxBlockSizeProperty<never>;
  inlineSize: T["tokens"]["sizes"] extends object ? keyof T["tokens"]["sizes"]
    : InlineSizeProperty<never>;
  minInlineSize: T["tokens"]["sizes"] extends object
    ? keyof T["tokens"]["sizes"]
    : MinInlineSizeProperty<never>;
  maxInlineSize: T["tokens"]["sizes"] extends object
    ? keyof T["tokens"]["sizes"]
    : MaxInlineSizeProperty<never>;
  width: T["tokens"]["sizes"] extends object ? keyof T["tokens"]["sizes"]
    : WidthProperty<never>;
  minWidth: T["tokens"]["sizes"] extends object ? keyof T["tokens"]["sizes"]
    : MinWidthProperty<never>;
  maxWidth: T["tokens"]["sizes"] extends object ? keyof T["tokens"]["sizes"]
    : MaxWidthProperty<never>;
  height: T["tokens"]["sizes"] extends object ? keyof T["tokens"]["sizes"]
    : HeightProperty<never>;
  minHeight: T["tokens"]["sizes"] extends object ? keyof T["tokens"]["sizes"]
    : MinHeightProperty<never>;
  maxHeight: T["tokens"]["sizes"] extends object ? keyof T["tokens"]["sizes"]
    : MaxHeightProperty<never>;
  flexBasis: T["tokens"]["sizes"] extends object ? keyof T["tokens"]["sizes"]
    : FlexBasisProperty<never>;
  flex:
    | (T["tokens"]["space"] extends object ? keyof T["tokens"]["space"] : never)
    | [
      number,
      T["tokens"]["space"] extends object ? keyof T["tokens"]["space"]
        : string | number,
    ]
    | [
      number,
      number,
      T["tokens"]["sizes"] extends object ? keyof T["tokens"]["sizes"]
        : string | number,
    ];
  ...
  borderLeftStyle: T["tokens"]["borderStyles"] extends object
    ? keyof T["tokens"]["borderStyles"]
    : BorderLeftStyleProperty;
  borderRadius: T["tokens"]["radii"] extends object ? keyof T["tokens"]["radii"]
  ...
  boxShadow: T["tokens"]["shadows"] extends object
    ...
    : [
      ...
      string | number,
      string | number,
      T["tokens"]["colors"] extends object ? keyof T["tokens"]["colors"]
        : Color,
    ];
  textShadow: T["tokens"]["shadows"] extends object
    ? keyof T["tokens"]["shadows"]
    : TextShadowProperty;
  zIndex: T["tokens"]["zIndices"] extends object ? keyof T["tokens"]["zIndices"]
    : ZIndexProperty;
...
export interface TCss<T extends TConfig> {
  (
    ...styles: (TCssProperties<T> | string | boolean | null | undefined)[]
  ): string;
  getStyles: (
    callback: () => any,
  ) => {
